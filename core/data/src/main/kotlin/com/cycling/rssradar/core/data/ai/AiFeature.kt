...
enum class AiFeature(
    ...
    DEDUPE(
        ...
        entry = "批处理：每日任务对新文章做组内相似度判定。",
        presentation = "列表「同一事件还有 N 篇」折叠入口；阅读页顶部同源提示条。",
    ),
    QUALITY(
        dbValue = 11,
        category = AiCategory.CONTENT,
    ...
    FEED_RECOMMEND(
        ...
        entry = "订阅页顶部的「猜你想订」横条；每日任务预生成。",
        presentation = "横向卡片列表：站点名 + 推荐理由 + 一键预览订阅。",
    ),
    DISCOVER(
        dbValue = 18,
        category = AiCategory.DISCOVERY,
        ...
        entry = "信息流顶部「发现」入口（开关打开后出现）。",
        presentation = "单列探索流，关闭推荐打分，只保证话题跨度与质量下限。",
    ),
    TOPIC_GALAXY(
        dbValue = 19,
        category = AiCategory.DISCOVERY,
        ...
        entry = "订阅页「话题星系」入口；依赖智能分类产物。",
        presentation = "话题簇列表：话题名 + 文章数 + 你的关联强度，点击进入话题文章流。",
    ),
    BUBBLE_BREAK(
        dbValue = 20,
        category = AiCategory.DISCOVERY,
        ...
        entry = "每日任务生成盲区清单；信息流穿插「破壁」卡片。",
        presentation = "信息流中的「换个视角」卡片组 + 破壁报告页（列出被忽略的话题）。",
    ),
    RELATED(
        dbValue = 21,
        category = AiCategory.DISCOVERY,
    ...
    AGGREGATE(
        ...
        entry = "话题星系页的「生成综述」；每日任务对热点话题预生成。",
        presentation = "综述页：共识 / 分歧 / 待观察三段式，附来源清单。",
    ),
    INTEREST_RANK(
        dbValue = 23,
        category = AiCategory.DISCOVERY,
        ...
        entry = "每日任务；兴趣画像页「重新解读」。",
        presentation = "兴趣画像页顶部的兴趣榜：命名话题 + 强度条 + 代表订阅源。",
    ),
    EVENT_MERGE(
        dbValue = 24,
        category = AiCategory.DISCOVERY,
        ...
        entry = "去重产物里的「合并阅读」入口。",
        presentation = "事件时间线页：按时间排列的报道节点，标注各源差异点。",
    ),
    COLD_START(
        dbValue = 25,
        category = AiCategory.DISCOVERY,
        ...
        entry = "首次进入推荐 tab 且画像为空时的引导卡；设置页可重做。",
        presentation = "领域勾选卡 → 立即生效的推荐流，勾选内容写入画像种子。",
    ),
    ...
    DAILY_BRIEF(
        ...
        entry = "每日任务生成；通知栏点击进入简报页。",
        presentation = "简报页：日期 + 要闻列表 + 每条附原文链接；历史简报可回看。",
    ),
    SHARE_COPY(
        dbValue = 27,
        category = AiCategory.ASSIST,
    ...
    SMART_NOTIFY(
        ...
        entry = "自动同步后由批处理判定；依赖全局通知开关与订阅源通知开关。",
        presentation = "仅重要文章进系统通知；被过滤掉的在通知历史里可查。",
    ),
    FEED_HEALTH(
        dbValue = 29,
        category = AiCategory.ASSIST,
        ...
        entry = "每日任务；订阅页的「健康检查」入口。",
        presentation = "健康报告页：按源列出状态（正常/降频/失效）+ 处置建议 + 一键跳转。",
    ),
    HABIT(
        dbValue = 30,
        category = AiCategory.ASSIST,
    ...
    DAILY_REPORT(
        ...
        entry = "每日任务生成，次日通知；「我的」页可回看历史报告。",
        presentation = "报告页：数字（真实统计）+ 模型写的一段总结 + 遗漏清单。",
    ),
    FILTER_RULE(
        dbValue = 32,
        category = AiCategory.ASSIST,
        ...
        entry = "设置页「过滤规则」入口；订阅源操作页可按源生成。",
        presentation = "规则预览列表（含命中示例），确认后启用；规则本身是纯本地匹配。",
    ),
    USAGE(
        dbValue = 33,
        category = AiCategory.ASSIST,
    ...
    PROMPT_TEMPLATE(
        ...
        entry = "设置 → AI 与诊断 → 「提示词」入口；订阅源操作页配单源提示词。",
        presentation = "模板页：功能列表 → 编辑框 → 变量说明 + 实时预览渲染结果。",
    ),
    ;
