...
object AiParsers {
    ...
    /**
     * 这次生成**值不值得存**——解析没报错，但主要字段全空，等于白跑一次。
     *
     * 模型最常见的软失败不是报错，而是回一个 `{"tags": []}` 之类的空壳：
     * 语法完全合法，解析一路绿灯，用户却看到一片空白还以为功能坏了。
     * 判空后不入库、记为失败，队列会按退避重试，用户看到的是"生成失败"而不是"生成了个寂寞"。
     */
    fun isMeaningful(feature: AiFeature, parsed: Any): Boolean = when (parsed) {
        is String -> parsed.isNotBlank()
        is AiTagsPayload -> parsed.tags.isNotEmpty()
        is AiKeywordsPayload -> parsed.keywords.isNotEmpty()
        is AiClassifyPayload -> parsed.topic.isNotBlank()
        is AiFulltextPayload -> parsed.ok
        is AiQaPayload -> parsed.answer.isNotBlank()
        is AiGlossaryPayload -> parsed.explanation.isNotBlank()
        is AiSharePayload -> parsed.variants.isNotEmpty()
        is AiBriefPayload -> parsed.items.isNotEmpty() || parsed.headline.isNotBlank()
        is AiDiscoverPayload -> parsed.articleIds.isNotEmpty()
        is AiTopicsPayload -> parsed.topics.isNotEmpty()
        is AiFeedRecommendPayload -> parsed.suggestions.isNotEmpty()
        is AiBubblePayload -> parsed.blindSpots.isNotEmpty() || parsed.articleIds.isNotEmpty()
        is AiColdStartPayload -> parsed.seeds.isNotEmpty()
        is AiEventPayload -> parsed.event.isNotBlank() && parsed.timeline.isNotEmpty()
        is AiAggregatePayload -> parsed.consensus.isNotEmpty() || parsed.divergence.isNotEmpty()
        is AiInterestRankPayload -> parsed.interests.isNotEmpty()
        is AiFilterRulePayload -> parsed.rules.isNotEmpty()
        else -> true
    }
     ...
     * 按功能解析模型原文，返回对应的载荷对象。
     *
     * 调用方（[AiFeatureRunner]）拿 [Any] 后自行转型——这里刻意不做泛型收口：
     * 35 种载荷类型没有共同父接口（也不该有，它们的字段毫无共性），
     * 硬造一个接口只会让每个分支都得判空转型。
     */
    fun parse(feature: AiFeature, raw: String): Any = when (feature) {
        AiFeature.TAGS -> tags(raw)
        AiFeature.KEYWORDS -> keywords(raw)
        AiFeature.CLASSIFY -> classify(raw)
        AiFeature.SENTIMENT -> sentiment(raw)
        AiFeature.QUALITY -> quality(raw)
        AiFeature.NOISE -> noise(raw)
        AiFeature.CREDIBILITY -> credibility(raw)
        AiFeature.OUTLINE -> outline(raw)
        AiFeature.OPINION -> opinion(raw)
        AiFeature.DEDUPE -> dedupe(raw)
        AiFeature.FULLTEXT -> fulltext(raw)
        AiFeature.GLOSSARY -> glossary(raw)
        AiFeature.QA -> qa(raw)
        AiFeature.FEED_RECOMMEND -> feedRecommend(raw)
        AiFeature.TOPIC_GALAXY -> topics(raw)
        AiFeature.BUBBLE_BREAK -> bubble(raw)
        AiFeature.AGGREGATE -> aggregate(raw)
        AiFeature.INTEREST_RANK -> interestRank(raw)
        AiFeature.EVENT_MERGE -> event(raw)
        AiFeature.DISCOVER -> discover(raw)
        AiFeature.COLD_START -> coldStart(raw)
        AiFeature.DAILY_BRIEF -> dailyBrief(raw)
        AiFeature.SHARE_COPY -> shareCopy(raw)
        AiFeature.SMART_NOTIFY -> importance(raw)
        AiFeature.FEED_HEALTH -> feedHealth(raw)
        AiFeature.HABIT -> habit(raw)
        AiFeature.DAILY_REPORT -> dailyReport(raw)
        AiFeature.FILTER_RULE -> filterRule(raw)

        // 纯本文产物与非 LLM 功能：不解析，原样返回。
        AiFeature.SUMMARY,
        AiFeature.TRANSLATE,
